...
try{
 ...
 await p.waitForFunction(()=>fourSnapshot().state.seats[1]?.connected);await p.locator('#ready').click();await q.waitForFunction(()=>fourSnapshot().state.seats[0].ready);await q.locator('#ready').click();await p.waitForFunction(()=>fourSnapshot().state.status==='playing');
