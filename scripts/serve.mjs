...
createServer(async (req, res) => {
  ...
  try {
    ...
    if(requestPath==='/assets/submission-config.js'&&process.env.SUBMISSION_ENDPOINT){res.writeHead(200,{'Content-Type':'text/javascript','Cache-Control':'no-store'});return res.end('window.ArcadeSubmissions={endpoint:'+JSON.stringify(process.env.SUBMISSION_ENDPOINT)+'};');}
    if(/^\/(?:multiplayer|submissions|node_modules|output|scripts)(?:\/|$)/i.test(requestPath)){res.writeHead(404);return res.end('Not found');}
