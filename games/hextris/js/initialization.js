...
function initialize(a) {
	...
	window.op = 0;
	window.saveState = hextrisStorage.getItem("saveState") || "{}";
	if (saveState !== "{}") {
		op = 1;
	}
	...
	highscores = [];
	if (hextrisStorage.getItem('highscores')) {
		try {
			highscores = JSON.parse(hextrisStorage.getItem('highscores'));
		} catch (e) {
			highscores = [];
		}
	}
	...
	if (a != 1) {
		...
		$(window).unload(function() {
			...
			if (gameState == 1 || gameState == -1 || gameState === 0) hextrisStorage.setItem("saveState", exportSaveState());
			else hextrisStorage.setItem("saveState", "{}");
		});
