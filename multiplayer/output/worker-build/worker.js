...
var worker_default = { async fetch(req, env) {
  ...
  try {
    if (req.method === "POST" && url.pathname === "/rooms") {
      ...
      const id = crypto.randomUUID(), room = env.ROOMS.get(env.ROOMS.idFromName(id));
      const response2 = await room.fetch("https://room/create", { method: "POST" });
      return json({ ...await response2.json(), room: id }, response2.status, origin);
    }
    ...
    return json({ error: "Room service is unavailable or at its quota. Your solo game is still available." }, 503, origin);
  }
} };
var FourRoom = class {
  static {
  ...
  async fetch(req) {
    ...
    if (path === "/gate") {
      ...
      if (this.gate.count >= 10) return json({}, 429);
      this.gate.count++;
    ...
    if (path === "/create" && req.method === "POST") {
      ...
      await this.save();
      return json({ token: this.room.seats[0].token, seat: 1 });
    }
    ...
    if (path === "/join" && req.method === "POST") {
      ...
      await this.save();
      this.broadcast();
      return json({ token: this.room.seats[1].token, seat: 2 });
    }
